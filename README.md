...
We're excited to have you here. This is a machine learning competition where you'll build a model to predict future price movements based on Limit Order Book (LOB) data and recent trades. It’s a challenging sequence modeling problem from the domain of high-frequency trading. Let's get started!
...
*   `need_prediction`: A boolean that’s `True` if we need a prediction from you for this step, and `False` otherwise.
...
## How to place well (practical plan)

A strong submission typically comes from being disciplined about validation, optimizing for the **Weighted Pearson** metric, and keeping inference fast and deterministic.

- **Validate by `seq_ix` (avoid leakage):** use `valid.parquet` as a holdout, and/or do K-fold splits by `seq_ix`.
- **Train for correlation, not just MSE:** the score weights by `abs(target)`; consider weighted losses (e.g., weights `abs(y)`), and use output squashing (e.g., `6 * tanh(x)`) so clipping doesn’t distort your predictions.
- **Exploit the warm-up window:** steps `0–98` are free context; design your model/windowing so it has enough history before step `99`.
- **Sequence models usually win here:** start with GRU/LSTM/Transformer-style models, then iterate on context length, normalization, and regularization.
- **Optimize CPU inference early:** keep per-step work small (fixed window, preallocated arrays), and consider ONNX for predictable CPU speed.
- **Don’t overfit the public board:** treat the Public Leaderboard as noisy feedback; keep a local validation you trust.
...
For more detail, see `docs/data.md` (dataset details) and `docs/submision.md` (submission format + limits).
